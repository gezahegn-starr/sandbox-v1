...
// Container represents the top-level container object returned by `container ls --format json`.
...
// workspacePath returns the first host-side mount source, used as the workspace label.
func (c *Container) workspacePath() string {
...
// isSandbox returns true if the container name follows the copilot-* naming convention.
func (c *Container) isSandbox() bool {
	id := c.Config.ID
	return len(id) > 8 && id[:8] == "copilot-"
}
